...
if subfolders:
    ...
    # Check if 'h' file exists in the highest folder
    h_file_path = os.path.join(highest_folder_path, 'h')
    if os.path.exists(h_file_path):
        print(f"Opening 'h' file in folder {highest_folder}:")
        ...
        os.chdir('Storage h/')
        os.chdir(f'T{num}')
        filename = f'h{numbers}'
        np.save(filename, h)
        os.chdir('..')
    else:
        print(f"'h' file not found in folder {highest_folder}.")
else:
