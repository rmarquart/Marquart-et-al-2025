...
# Change directory to 'Previous index values/h2/T{num}' folder
...
for i, category in enumerate(greaseIceCategoryAverages):
    ...
    if np.isnan(category):
        ...
        print(f'entry of h2 in category {i}: 0')
        print(f'outcome of h2 in category {i}: 0')

    else:

        filename = f'prev_index_values_{i}.pkl'

        # Print the name of the file that will be opened      
        ...
        hi = greaseIceCategoryAverages[i]
        h2 = hi
        print(f'entry of h2 in category {i}: ', "{:.6f}".format(h2))
        ...
        L0i = 234000.14                          # latent heat of fusion of pure ice ()
        ...
        mui_bio = deltat / (0.916 * 10**3 * cpi_bio * loaded_variables['hi_bio'])
        ...
        for j in range(1, maxIterations):
            ...
            mui = deltat / (0.916 * 10**3 * cpi_it * hi)
        ...
        Qi_surf = 0.916 * 10**3 * ((c0 * (Tfr - Ti)) + L0s * (1 + mu * loaded_variables['Si'] / (Ti - 273.15)))
        Qi_bott = 0.916 * 10**3 * ((c0 * (Tfr - Ti)) + L0s * (1 + mu * loaded_variables['Si'] / (Ti - 273.15)))
        ...
        if T0 < Tfr:
            if loaded_variables['hmi'] > hmi_min:
                ...
                if (loaded_variables['hs'] + hs_prec_bucket) > hs_min:
                    ...
                    if (loaded_variables['hs'] + hs_prec_bucket - delta_sublim) > (hi * (row - 0.916 * 10**3) / ros_av - hs_prec_bucket * ros_new / ros):
                        hs = loaded_variables['hs'] + delta_bucket - delta_sublim - beta * ((-row * hi + ros * loaded_variables['hs'] + 10**3 * 0.916 * hi + ros_new * hs_prec_bucket) / (-romi + row + beta * ros_av))
                        hmi_new = ((-row * hi + ros * (loaded_variables['hs'] - delta_sublim) + 10**3 * 0.916 * hi + ros_new * hs_prec_bucket) / (-romi + row + beta * ros_av)) * si_fract
                    ...
                    if hi > hi_min:
                        delta_sublim = (Fla * deltat) / (0.916 * Lv - Qi_surf)
                        deltahi_bott = deltat * (Fbott - Fw - ISI) / Qi_bott
                    else:
                        delta_sublim = 0.0
                    ...
                    if hi < ZERO:
                        R = (0.916 * 10**3 * ((c0 * (Tfr - Ti)) + L0s * (1 + loaded_variables['Si'] / (Ti - 273.15)))) * (hi - ZERO) / deltat
                        hi = ZERO
                        hs = ZERO
                        hmi = ZERO
            ...
            if hi > ZERO:
                ...
                if hmi_new > 0.0:
                    snow_fr = beta / (0.916 * 10**3 / ros_av)
            ...
            if (loaded_variables['hs'] + hs_prec_bucket) > hs_min:
                ...
                if loaded_variables['hmi'] > hmi_min:
                    ...
                    if hi > ZERO:
                        ...
                        if hi > ZERO:
                            if loaded_variables['Vbr_ice'] >= 0.05:
                                Q = 0.20 * 0.916 * (-deltahi_melt_surf) / deltat
                                Sbr_ice_star = loaded_variables['Sbr_ice'] - ((Q / (loaded_variables['Vbr_ice'] * loaded_variables['ro_br'] * 10**3)) * (loaded_variables['Sbr_ice'] - Sbr_end)) * deltat
                                Sice = loaded_variables['Sice'] + (Sbr_ice_star - loaded_variables['Sbr_ice']) * loaded_variables['Vbr_ice']
                                Vbr_ice = -mu * Sice / Tice
                                Sbr_ice = Sice / Vbr_ice
        ...
        h2 = hi + dhG
        print(f'outcome of h2 in category {i}: ', "{:.6f}".format(h2))
        ...
        loaded_variables = {
            ...
            'ks_snow': ks_snow, 'alpha': alpha, 'ki_5_bott': ki_5_bott, 'ki_ice_5': ki_ice_5, 'ki_ice_bio': ki_ice_bio, 'ki_bio_bott': ki_bio_bott, 'ki_5_bio': ki_5_bio, 
        ...
        column_name = f'h2_cat{i}'
        df.at[0, column_name] = h2
